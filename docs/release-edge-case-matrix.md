...
|---|---|---:|---|
| Onboarding | Complete or Skip persists; relaunch opens Home | Partial | focused UI relaunch test |
| Player count | 1 blocked; 2 and 15 allowed; 16 blocked | Gap | pure validation test |
| Player names | Empty and whitespace-only blocked; trim cannot bypass duplicate check | Partial | pure validation test |
| Generic scoring | Direct Ledger Rail submit creates one round with one entry per player | Partial | SwiftData integration + one UI smoke |
| Round persistence | Submitted round, totals, and next round survive relaunch | Gap | SwiftData integration |
...
| Undo | Removes only latest round, restores totals, and persists | Gap | integration + one UI smoke |
| Phase progression | Complete advances once; failure follows skip option; phase caps at 10 | Gap | pure engine tests |
| Phase completion alert | Keep Playing and End Game preserve their distinct contracts | Gap | focused UI test |
| Dinner | Caller and points persist; caller resets; lowest total wins | Partial | engine/integration + UI smoke |
| Resume | Active session and exact round resume after termination | Partial | relaunch UI test |
| Rematch | Copies mode, settings, and players; copies no rounds; consumes one allowance | Partial | integration + UI gate |
...
| Stats | Games, wins, average, best rank, ties, and no-data values are correct | Gap | pure unit tests |
| Pro purchase | Verified purchase unlocks and persists; pending/unverified do not unlock | Partial | StoreKit adapter/local configuration |
| Restore | Entitlement restores; no purchase stays locked; failure remains retryable | Gap | StoreKit adapter/local configuration |
| Review ask | Counts 2 and 5 only; first session, 120-day, paywall, and duplicate gates | Partial | pure manager tests |
| Accessibility | Critical controls have useful label, value, and action | Partial | focused accessibility UI queries |
| Dynamic Type | Setup, scoring, paywall, Game Over, and history keep actions reachable | Gap | one accessibility size UI pass + screenshots |

## P1 before TestFlight review

| Area | Contract | Current | Proof layer |
|---|---|---:|---|
| Lowest-score game | Stored rule changes completion and winner behavior | Partial | engine + one UI flow |
| Duplicate submit | Rapid repeat cannot create two rounds | Gap | integration |
| End-game cancel | Entered state and active session remain unchanged | Gap | focused UI test |
| Roster | Cancel changes nothing; confirmed deletion persists; games retain players | Partial | integration + UI cancel |
| History detail | Correct game, standings, and rounds open from Home and History | Partial | focused UI test |
| Head to Head | Empty, selected, expanded, collapse, and mixed-mode grouping are safe | Partial | calculator unit + UI expansion |
| Tools | Timer, dice, starter, undo, and log never corrupt game state | Gap | helper tests + UI smoke |
| Theme | Light, dark, and system selection persists across relaunch | Partial | manager/integration |
| Reduce Motion | No confetti or movement fallback; data and navigation timing unchanged | Gap | UI + manual motion review |
| Corrupt metadata | Invalid phase JSON and unknown enum values recover without crash | Gap | model unit tests |
| Handwriting retirement | Legacy launch flag cannot restore the retired handwriting flow | Gap | UI regression, RED before code |
