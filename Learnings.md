...
- On July 15, the six 1320×2868 storefront screenshots were regenerated from build 7's PipCount/Ten Phases UI, validated with zero errors or warnings, and replaced successfully in App Store Connect.
...
- PipCount 1.0 build 7 was archived from pushed commit `dfb1f66` with stable Xcode 26.6, verified as signed by team `4JRB53LG5C`, uploaded through the ASC Admin-key profile, processed as valid, and attached to App Store version 1.0.
- Build 7 is in the `PipCount External Testers` group and is `WAITING_FOR_REVIEW` in Beta App Review. Enable the public link only after approval. App Store version review remains separate and must wait for the privacy, agreements, and first-IAP-with-version gates.
