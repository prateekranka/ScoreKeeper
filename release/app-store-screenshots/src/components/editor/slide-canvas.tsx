    ...
    <div style={{ textAlign: align, position: "relative", width: "100%" }}>
      ...
      <EditableText
        ...
        style={{
          fontSize: unit * 0.092,
          fontWeight: 700,
          fontFamily: theme.id === "pipcount-scorecard" ? '"Press Start 2P", monospace' : undefined,
          lineHeight: theme.id === "pipcount-scorecard" ? 1.18 : 0.96,
...
function backgroundFor(theme: Theme, inverted?: boolean) {
  if (theme.id === "pipcount-scorecard") {
...
function SlideBackground({
  ...
  return (
    ...
    >
      {theme.id === "pipcount-scorecard" ? (
        <PixelScorecardAccents cW={cW} cH={cH} inverted={inverted} />
      ) : (
        <>
...
function PixelScorecardAccents({ cW, cH, inverted }: { cW: number; cH: number; inverted: boolean }) {
  const ink = inverted ? "#FFF9ED" : "#20211D";
  const squares = [
    { left: 0.055, top: 0.035, color: "#2F7050" },
    { left: 0.083, top: 0.035, color: "#9B7620" },
    { left: 0.91, top: 0.91, color: "#A6493E" },
    { left: 0.882, top: 0.91, color: "#2C64A0" },
  ];
  return (
    <>
      <div aria-hidden style={{ position: "absolute", left: cW * 0.05, right: cW * 0.05, top: cH * 0.045, height: 2, background: ink, opacity: 0.14 }} />
      {squares.map((square, index) => (
        <div
          ...
          style={{
            ...
            boxShadow: `0 ${Math.max(3, cW * 0.004)}px 0 ${ink}22`,
          }}
        />
      ))}
    </>
