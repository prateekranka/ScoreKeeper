...
const FINAL_ARTBOARD_NAME = "ScoreKeeper Cup Hybrid A - Articulated Rig v1";
const TEMP_ARTBOARD_NAME = `__BUILDING__ ${FINAL_ARTBOARD_NAME}`;
const STATE_MACHINE_NAME = STATE_MACHINE_SPEC.name;
const STATE_LAYER_NAME = STATE_MACHINE_SPEC.layerName;
const SUMMARY_PATH = join(ROOT, "rive-build-summary.json");
const PAPER = "#fff4f2ec";
const ARTBOARD_X = 832;
const ARTBOARD_Y = 0;
...
async function initialize() {
  await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "codex-scorekeeper-cup-rig-v1", version: "1.0" },
  });
...
function validateSourceSpecs() {
  ...
  assert(CANVAS.width === 512 && CANVAS.height === 416, `Expected a 512x416 artboard, found ${CANVAS.width}x${CANVAS.height}`);
  assert(FPS === 60, `The Rive contract requires 60fps, found ${FPS}`);
  assert(NODES.length === PIVOTS.length + PARTS.length, "NODES/PIVOTS/PARTS disagree");
  assert(NODES.length === 185 && PIVOTS.length === 3 && PARTS.length === 182, "Rig inventory changed after approval");
  assert(HAIR_PART_NAMES.length === 6, `Expected six native hair vectors, found ${HAIR_PART_NAMES.length}`);
  assert(PERFORMANCE_SPECS.length === 3, `Expected three performances, found ${PERFORMANCE_SPECS.length}`);
  ...
  for (const spec of PERFORMANCE_SPECS) {
    assert(PERFORMANCE_BY_SLUG[spec.slug]?.displayName === spec.displayName, `${spec.slug}: PERFORMANCE_BY_SLUG is inconsistent`);
    assert(Number.isFinite(spec.durationSeconds) && spec.durationSeconds > 0, `${spec.displayName}: invalid duration`);
    assert(terminalFrame(spec) === Math.round(spec.durationSeconds * FPS), `${spec.displayName}: terminal frame mismatch`);
    for (const [pivotName, properties] of Object.entries(spec.tracks)) {
      assert(BASE_BY_NAME[pivotName], `${spec.displayName}: track targets unknown node ${pivotName}`);
      for (const [property, keys] of Object.entries(properties)) {
        assert(PROPERTY[property], `${spec.displayName}/${pivotName}: unsupported property ${property}`);
        assert(Array.isArray(keys) && keys.length >= 2, `${spec.displayName}/${pivotName}/${property}: requires at least two keys`);
        assert(keys[0][0] === 0, `${spec.displayName}/${pivotName}/${property}: missing frame 0`);
        assert(keys.at(-1)[0] === terminalFrame(spec), `${spec.displayName}/${pivotName}/${property}: missing terminal frame`);
        for (let index = 0; index < keys.length; index += 1) {
          const [frame, value] = keys[index];
          assert(Number.isInteger(frame), `${spec.displayName}/${pivotName}/${property}: non-integer frame ${frame}`);
          assert(frame >= 0 && frame <= terminalFrame(spec), `${spec.displayName}/${pivotName}/${property}: frame ${frame} out of bounds`);
          assert(Number.isFinite(value), `${spec.displayName}/${pivotName}/${property}: non-finite value at ${frame}`);
          if (index > 0) {
            assert(frame > keys[index - 1][0], `${spec.displayName}/${pivotName}/${property}: frames must increase strictly`);
          }
        }
        if (spec.loop) {
          assert(
            closeEnough(keys[0][1], keys.at(-1)[1]),
            `${spec.displayName}/${pivotName}/${property}: loop endpoints differ`,
          );
        }
      }
    }
...
async function setArtboardFillColor(artboardId, color = PAPER) {
...
async function createRigGeometry(artboardId) {
  ...
  for (const node of PIVOTS) {
    ...
    const shape = transparentPivotShape(node, parentId);
  ...
  const leafShapes = PARTS.map((node) => {
    const parentId = idsByName.get(node.parent);
    assert(parentId, `${node.name}: unresolved parent ${node.parent}`);
    return visiblePartShape(node, parentId);
  });
  for (const group of chunks(leafShapes, SHAPE_CHUNK_SIZE)) {
    await focusLock(artboardId);
    await tool("path_editor", {
      command: "createShapes",
      data: { createShapes: { shapes: group } },
    });
  }
...
async function verifyRigGeometry(artboardId, expectedIdsByName) {
  ...
  const animatedNodeNames = [...new Set(PERFORMANCE_SPECS.flatMap((spec) => Object.keys(spec.tracks)))];
  const animatedNodeIds = animatedNodeNames.map((name) => actualIdsByName.get(name));
  for (const group of chunks(animatedNodeIds, PROPERTY_CHUNK_SIZE)) {
    const schema = await tool("query_property_keys", { objectIds: group, animates: true });
    assertPropertySchema(schema, group, TRANSFORM_PROPERTY_KEYS, "Animatable rig-node schema");
  }
...
function riveKeyframesForSpec(spec, idsByName) {
  ...
  for (const [pivotName, properties] of Object.entries(spec.tracks)) {
    ...
    assert(objectId, `${spec.displayName}: missing Rive ID for ${pivotName}`);
    for (const [property, keys] of Object.entries(properties)) {
...
function verifyReportedDuration(animation, spec) {
  ...
  assert(Number.isFinite(numeric), `${spec.displayName}: Rive returned invalid duration ${reported}`);
  const isSeconds = closeEnough(numeric, spec.durationSeconds, 1e-3);
  const isFrames = closeEnough(numeric, terminalFrame(spec), 1e-3);
  assert(isSeconds || isFrames, `${spec.displayName}: duration expected ${spec.durationSeconds}s/${terminalFrame(spec)}f, found ${reported}`);
}
...
async function createAnimations(artboardId, idsByName) {
  ...
  for (const [animationId, expectedEntry] of expectedByAnimationId) {
    ...
    assert(
      ...
      `${spec.displayName}: expected ${expected.length} keyframes, found ${actual.length}`,
    );
    ...
    for (const keyframe of actual) {
      ...
      assert(!actualByTuple.has(tuple), `${spec.displayName}: duplicate queried keyframe tuple ${tuple}`);
    ...
    for (const keyframe of expected) {
      ...
      assert(found, `${spec.displayName}: missing queried keyframe ${tuple}`);
      assert(
        closeEnough(found.value, keyframe.value, 1e-4),
        `${spec.displayName}: ${tuple} expected value ${keyframe.value}, found ${found.value}`,
      );
    }
    ...
    for (const [pivotName, properties] of Object.entries(spec.tracks)) {
      ...
      for (const [property, keys] of Object.entries(properties)) {
        ...
        assert(first && last, `${spec.displayName}/${pivotName}/${property}: queried track lacks an endpoint`);
        if (spec.loop && !closeEnough(first.value, last.value, 1e-4)) {
          loopErrors.push(`${pivotName}/${property}`);
        }
        assert(keys[0][0] === 0 && keys.at(-1)[0] === terminal, `${spec.displayName}/${pivotName}/${property}: authored endpoints regressed`);
      }
    }
    assert(loopErrors.length === 0, `${spec.displayName}: loop endpoint mismatch on ${loopErrors.join(", ")}`);
    verification.push({
      slug: spec.slug,
      name: spec.slug,
...
function stateDefinitions() {
  const definitions = STATE_MACHINE_SPEC.states.map((state) => ({
    stateName: state.name,
    animationName: state.animation,
    x: state.x,
    y: state.y,
  }));
  assert(new Set(definitions.map((definition) => definition.stateName)).size === 3, "State names are not unique");
...
async function createAndVerifyStateMachine(artboardId, animationBySlug) {
  ...
  assert(namedStates.length === definitions.length, `Behavior layer expected three named states, found ${namedStates.length}`);
  ...
  return {
    ...
    invocation: "Play Celebrate or Hair Sway by exact linear-animation name; this mapping-only state machine intentionally creates no runtime inputs.",
  };
}
...
async function build() {
  ...
  const summary = {
    ...
    palette: { artboardPaper: PAPER, artwork: "source-locked canonical SVG palette" },
    fps: FPS,
    rig: {
      ...
      hairComponent: {
        controlName: "rig_hair",
        controlId: finalGeometry.idsByName.get("rig_hair"),
        nativeVectorChildNames: HAIR_PART_NAMES,
        nativeVectorChildIds: HAIR_PART_NAMES.map((name) => finalGeometry.idsByName.get(name)),
        nativeVectorChildCount: HAIR_PART_NAMES.length,
        seamUnderlapName: "rig_body__hair_underlap",
        seamUnderlapId: finalGeometry.idsByName.get("rig_body__hair_underlap"),
      },
    },
