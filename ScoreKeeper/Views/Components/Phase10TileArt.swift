...
/// Bitmap replacement for the hand-drawn Ten Phases tile thumbnail: the bundled
/// card-fan artwork with a subtle idle breathing motion. The parent supplies the
/// TimelineView time, drives `active`, and clips the surrounding tile.
struct Phase10TileArt: View {
    let size: CGSize
...
/// view's index-0 artwork, minus rotation (the bitmap must never rotate).
/// Applied outside the breathing scale/offset so the two motions don't fight.
